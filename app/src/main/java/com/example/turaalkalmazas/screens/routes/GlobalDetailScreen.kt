...
@Composable
fun GlobalDetailScreen(
    routeId: String,
    restartApp: (String) -> Unit,
    viewModel: GlobalDetailViewModel = hiltViewModel()
) {
    LaunchedEffect(Unit) {
    ...
    Column(
        ...
        Text(
            text = "Distance: ${
                routeDetails.length.takeIf { it.isNotEmpty() }
                    ?.toDoubleOrNull()
                    ?.let { "%.0f".format(it) } ?: 0
            } km",
            fontSize = 18.sp,
        ...
        Text(
            text = "Height Difference: ${routeDetails.altitudeDiff} m",
            fontSize = 18.sp,
            modifier = Modifier.padding(bottom = 4.dp)
        )
